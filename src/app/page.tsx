...
export default function Home() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
        // STEP 1: Fetch Emails
        const emailRes = await fetch(`/api/emails?sources=${encodeURIComponent(allowedSources)}`);
        ...
        if (emailData.emails.length === 0) {
            setResult(`No emails found matching the sources: ${allowedSources}. Please check your filters.`);
            setLoading(false);
            return;
        }
        ...
        const calendarRes = await fetch('/api/calendar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ events: analyzedData.events, summary: analyzedData.summary }),
        });
  ...
  if (!user) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
        ...
        <p className="mb-6 text-gray-600 text-center">Please sign in to access your email analysis and scheduling tool.</p>
        <button
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-8 sm:p-12">
      ...
      <p className="text-xl font-light text-gray-600 mb-8">
        We'll find the dates, tasks, and key announcements from your filtered emails and schedule them automatically.
      </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 shadow-xl rounded-xl">
          ...
          <div className="flex space-x-2 mb-4">
              ...
              <button
                  ...
                  className={`px-4 py-3 rounded-lg text-white font-medium transition duration-150 ${
                      saveStatus === 'saving' ? 'bg-gray-400' : 'bg-violet-600 hover:bg-violet-700'
                  }`}
              >
          ...
          {saveStatus === 'error' && (
              <p className="text-sm text-red-600 mb-4">Failed to save filters.</p>
          )}
